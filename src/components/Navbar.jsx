...
import {oussama,menu,close} from '../assets';
const Navbar = () => {
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
        <img src={oussama} alt="logo" className='w-9 h-9 object-contain' />
        <p className='text-white text-[18px] font-bold cursor-pointer flex'>OUSSAM
        ...
        <ul className='list-none hidden sm:flex flex-row gap-10 '>
          {navLinks.map((link)=>(
            <li key={link.id} className={`${active==link.title 
              ? 
              "text-white":"text-secondary"} hover:text-pink text-[18px] font-medium cursor-pointer`}
